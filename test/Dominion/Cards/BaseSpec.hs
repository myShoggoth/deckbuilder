...
spec = do
  ...
  describe "vassalCardAction" $
    it "draws a value card" $ do
      let forcedDeal = DominionPlayer "Vassal Deal" (replicate 5 copperCard) [] [vassalCard, estateCard, estateCard, copperCard, copperCard] [] 1 1 0 0 0 [] [] [] 0 0 bigMoneyStrategy
  ...
  describe "bureaucratCardAction" $ do
    let forcedDeal  = DominionPlayer "Bureaurat Deal" (replicate 5 copperCard) [] [vassalCard, estateCard, estateCard, copperCard, copperCard] [] 1 1 0 0 0 [] [] [] 0 0 bigMoneyStrategy
    let (Just p1AfterDeal) = afterDeal ^? #players . ix 0
  ...
  describe "throneRoomCardAction" $ do
    ...
    it "will play Smithy twice in the bigSmithyStrategy" $ do
      let forcedDeal = DominionPlayer "Throne Room Deal" (replicate 5 copperCard) [] [throneRoomCard, smithyCard, estateCard, copperCard, copperCard] [] 1 1 0 0 0 [] [] [] 0 0 bigSmithyStrategy
      let afterCard = execState (evaluateHand p0) $ DominionBoard [forcedDeal] (basicDecks 2) [] (Map.fromList [] :: Map.Map Card Int) [moatCard] curseCard g
      let (Just p1AfterCard) = afterCard ^? #players . ix 0
      (p1AfterCard ^. #actions) `shouldBe` 0
      length (p1AfterCard ^. #hand) `shouldBe` 0
  ...
  describe "banditCardAction" $ do
    ...
    it "trashes an opponent's silver" $ do
      let forcedDeal = DominionPlayer "Bandit Deal" (silverCard: replicate 5 copperCard) [] [estateCard, smithyCard, estateCard, copperCard, copperCard] [] 1 1 0 0 0 [] [] [] 0 0 bigSmithyStrategy
      let (Just p1AfterDeal)  = afterDeal ^? #players . ix 0
