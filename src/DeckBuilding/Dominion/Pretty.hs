...
instance Pretty DominionAction where
    ...
    pretty (Bureaucrat xs) = "Bureaucrat" <+> hsep (map bureaucratResponse $ Map.toList xs)
    ...
    pretty (CouncilRoom (DominionDraw xs) others) = "Council Room draws " <+> hsep (map pretty xs) <+> " and causes others to draw:" <+> hsep (map (\(k,v) -> "Player draws " <> pretty v) $ Map.toList others)
    ...
    pretty (Militia xs) = "Militia:" <+> hsep (map militiaResponse $ Map.toList xs)
    ...
    pretty (Witch (DominionDraw xs) ys) = "Witch draws" <+> hsep (map pretty xs) <+> "and each player:" <+> hsep (map witchResponse $ Map.toList ys)
